...
class AnchorTargetCreator(object):
    ...
    def __call__(self, bbox, anchor, img_size):
        ...
        label=_umap(label, n_anchor, inside_index, fill=-1)
        loc=_umap(loc, n_anchor, inside_index, fill=0)
...
def _umap(data, count, index, fill=0):
    if len(data.shape)==1:
        ret=np.empty((count,), dtype=data.type)
        ret.fill(fill)
        ret[index]=data
    else:
...
class ProposalCreator:
    ...
    def __call__(self, loc, score, anchor, img_size, scale=1.):
        ...
        keep=nms(t.from_numpy(roi).cuda(),
